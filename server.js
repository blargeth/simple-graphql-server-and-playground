...
// OUR "DATABASE"
// The "owners" and "pets" variables are storing our data locally, so as to avoid having to set up a database for this simple server. Notice the relationship between "owners" and "pets," where each pet has an owner indicated by the ownerId in each element of the pets array. I will refer to each of these variables as 'collections' (i.e. the pets collection and owners collection).
const owners = [
...
// NOW THAT WE'RE DONE SETTING UP EACH "TYPE" FOR EACH COLLECTION, WE CAN SET UP THE KINDS OF REQUESTS WE CAN MAKE TO OUR SERVER. ONE KIND OF REQUEST BELOW WILL BE A "QUERY," WHICH FUNCTIONS LIKE A GET REQUEST (IN A REST API), GRABBING AND RETURNING DATA FROM THE DATABASE FOR YOU. THE OTHER WE WILL SET UP WILL BE A "MUTATION," WHICH FUNCTIONS LIKE A POST REQUEST (IN A REST API), ADDING DATA TO THE DATABASE FOR YOU (IN OUR CASE, THE "DATABASE" CONSISTS OF THE OWNERS AND PETS VARIABLES ABOVE).

// This allows the frontend to make queries to our server to retrieve data from the database. If you look within the fields method, it indicates we could query for a single pet (and the args indicates we'll have to provide an id for the pet), we could query for all the pets, we could query for a single owner (and the args indicates we'll have to provide an id for the owner), and we could query for all the owners. Queries can also consists of a mixture of these (i.e. can query for all the owners and for each of them, show all the pets). Some of the query examples at the bottom of this file demonstrate this.
...
// This allows the frontend to make "mutations" to our server to add data to our database. If you look within the fields method, it indicates we could add a new pet (and the args indicates we'll have to provide a name and ownerId for the newly created pet), and we can add a new owner (and the args indicates we'll have to provide a name for the newly created owner).
...
// we set up an endpoint for our requests ...
app.use('/graphql', graphqlHTTP({
  schema: schema,
  graphiql: true // allows us to use the GraphiQL tool (analogous to Postman, except for GraphQL) to manually issue GraphQL queries
}));

// ... and listen for it on port 8080 in this case
app.listen(8080, () => {
  console.log('Running a GraphQL API server at http://localhost:8080/graphql');
});

// AT THIS POINT YOU CAN START YOUR SERVER ('npm install' and then 'npm run start' in your terminal from this repo's root directory, if you haven't done this already), THEN TYPE localhost:8080/graphql INTO YOUR BROWSER, HIT ENTER, AND THE GRAPHIQL TOOL (a tool like Postman, allowing you to make HTTP requests without having a built-out frontend) WILL POP UP IN YOUR BROWSER, WHERE YOU CAN TRY OUT VARIOUS QUERIES AND MUTATIONS AND SEE WHAT DATA YOU GET BACK FROM THIS SERVER.
// YOU CAN USE THE QUERIES/MUTATIONS BELOW IN THE GRAPHIQL TOOL IN YOUR BROWSER. YOU CAN ONLY RUN ONE QUERY/MUTATION AT A TIME, SO MAKE SURE THE OTHERS ARE COMMENTED OUT BEFORE YOU HIT THE RUN BUTTON IN THE GRAPHIQL TOOL (in the event you copy and paste all of the below in the tool at once, which is fine). FROM THERE YOU CAN JUST PLAY AROUND WITH THE QUERIES/MUTATIONS, AND THEN MAYBE EVEN PLAY AROUND WITH THE ABOVE CODE TO MODIFY THE SERVER AND THUS THE QUERIES/MUTATIONS YOU CAN MAKE.
